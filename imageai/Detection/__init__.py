...
class ObjectDetection:
    ...
    def detectCustomObjectsFromImage(self, custom_objects=None, input_image="", output_image_path="", input_type="file",
                                     ...
                                     display_object_name=False, thread_safe=False):
        """
        ...
        if (self.__modelLoaded == False):
            ...
            try:
                if (self.__modelType == "retinanet"):
                    ...
                    # useless_dir = output_image_path + "-useless"
                    # if (os.path.exists(useless_dir) == False):
                    #     os.mkdir(useless_dir)

                    for index, (label, score), in enumerate(zip(predicted_numbers, scores)):
                        if score < min_probability:
                            # useless_copy = detected_copy2.copy()
                            # useless_image_path =  os.path.join(useless_dir, os.path.basename(input_image))
                            # pltimage.imsave(useless_image_path, useless_copy)
                            continue
                        ...
                        if (extract_detected_objects == True):
                            splitted_copy = detected_copy2#.copy()
                            # [detection_details[1]:detection_details[3],
                                            # detection_details[0]:detection_details[2]]
                            if (output_type == "file"):
                                splitted_image_path = os.path.join(objects_dir, os.path.basename(input_image))
                                # self.numbers_to_names[label] + "-" + str(counting) + ".jpg")
                                pltimage.imsave(splitted_image_path, splitted_copy)
                                detected_objects_image_array.append(splitted_image_path)
                            elif (output_type == "array"):
                                detected_objects_image_array.append(splitted_copy)
